...
- Reuse the final signed APK when inputs have not changed:
  - `python3 src/orch/orchestrator.py compile --reuse-artifacts`
- Run the full integration test:
  - `python3 src/orch/orchestrator.py test --serial emulator-5554`
- Run the smoke test:
  - `python3 src/orch/orchestrator.py test --smoke --serial emulator-5554`
...
1. `python3 src/orch/orchestrator.py plan`
...
2. `python3 src/orch/orchestrator.py pre-cache`
...
3. `python3 src/orch/orchestrator.py compile-modules`
...
7. `python3 src/orch/orchestrator.py test --smoke --serial emulator-5554`
...
9. `python3 src/orch/orchestrator.py test --serial emulator-5554`
...
- `yarn flow:plan`
- `yarn flow:pre-cache`
- `yarn flow:compile-modules`
- `yarn flow:merge`
- `yarn flow:compile`
- `yarn flow:compile:reuse`
- `yarn flow:test`
...
- All dependency rules are defined in `src/orch/cache_manifest.json`.
- Module artifact builders are declared in `src/orch/cache_manifest.json` under `builder`.
