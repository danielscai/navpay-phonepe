...
**Tech Stack:** Python 3 (`src/cache-manager`), Bash scripts (`src/*/scripts`, `src/tools`), adb/apktool/zipalign/apksigner, Android emulator.
...
- Create: `src/tools/archive_apk.sh`
- Create: `src/tools/behavior_probe.sh`
- Create: `src/tools/compare_behavior.sh`
- Modify: `package.json`
- Test: `src/tools/README.md`
...
- `bash src/tools/archive_apk.sh --help`
- `bash src/tools/behavior_probe.sh --help`
- `bash src/tools/compare_behavior.sh --help`
Expected: PASS and scripts return usage help with code 0.

**Step 5: Commit**

```bash
git add artifacts src/tools package.json src/cache-manager/tests/test_artifact_contract.py
...
- Modify: `src/tools/lib/dispatcher.sh`
...
def test_phonepehelper_does_not_patch_hookentry_directly():
    text = Path("src/phonepehelper/scripts/merge.sh").read_text()
...
```bash
git add src/phonepehelper/scripts/merge.sh src/signature_bypass/scripts/inject.sh src/tools/lib/dispatcher.sh src/cache-manager/tests/test_entry_contract.py docs/hook_runtime_architecture.md
...
"baseline:archive": "bash src/tools/archive_apk.sh --label baseline",
"probe:baseline": "bash src/tools/behavior_probe.sh --input artifacts/baseline/latest/apk.apk --out artifacts/runs/baseline",
"probe:candidate": "bash src/tools/behavior_probe.sh --input cache/profile_full_build/patched_signed.apk --out artifacts/runs/candidate",
"probe:compare": "bash src/tools/compare_behavior.sh --base artifacts/runs/baseline --cand artifacts/runs/candidate"
```
...
```bash
git add docs/verification/behavior-parity-checklist.md src/tools/README.md package.json src/cache-manager/tests/test_behavior_parity_schema.py src/tools/archive_apk.sh src/tools/behavior_probe.sh src/tools/compare_behavior.sh
