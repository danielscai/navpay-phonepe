...
yarn release:to-admin \
  --env local \
  --apk cache/apps/phonepe/snapshot_seed/base.apk \
  --abi-apk cache/apps/phonepe/snapshot_seed/split_config.arm64_v8a.apk \
  --density-apk cache/apps/phonepe/snapshot_seed/split_config.xxhdpi.apk \
...
- 默认目标环境是 `local`。
- 默认输入应来自 `cache/apps/phonepe/snapshot_seed`，该目录由版本化 snapshot 快照生成，不再直接读取旧的 `from_device` 目录。
