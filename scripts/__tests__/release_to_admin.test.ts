...
test("fails fast when base/abi/density signatures are inconsistent", async () => {
  const tempDir = mkdtempSync(path.join(os.tmpdir(), "release-to-admin-test-"));
  const baseApkPath = path.join(tempDir, "base.apk");
  const abiApkPath = path.join(tempDir, "split_config.arm64_v8a.apk");
  const densityApkPath = path.join(tempDir, "split_config.xxhdpi.apk");
  ...
  try {
    ...
    await assert.rejects(
      runReleaseCli(
        ["--base-apk", baseApkPath, "--abi-apk", abiApkPath, "--density-apk", densityApkPath],
        fakeApi as any,
        {
          readApkMetadata: async () => ({
          ...
          sha256File: async () => "sha_base",
          readApkSigningDigest: async (apkPath: string) => {
            if (apkPath === baseApkPath) return "digest_base";
            if (apkPath === abiApkPath) return "digest_abi";
            return "digest_density";
          },
        } as any,
      ),
