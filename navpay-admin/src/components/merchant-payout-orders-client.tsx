...
export default function MerchantPayoutOrdersClient() {
  ...
  async function loadOrders() {
    ...
    const r = await fetch(`/api/merchant/orders/payout?${sp.toString()}`);
  ...
  const maskAcct = (s: string) => (s.length <= 6 ? s : `${s.slice(0, 2)}****${s.slice(-2)}`);
          ...
          <tbody>
            {orders.map((o) => (
              <tr key={o.id} className="border-t border-white/10">
                ...
                <td className="px-3 py-2 text-xs text-[var(--np-muted)]">
                  <div className="font-mono">{o.beneficiaryName}</div>
                  <div className="text-[var(--np-faint)]">{maskAcct(o.accountNo)} / {o.ifsc}</div>
                </td>
