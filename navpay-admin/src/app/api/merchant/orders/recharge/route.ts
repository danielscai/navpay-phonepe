...
export async function GET(req: NextRequest) {
  ...
  const conds: any[] = [eq(rechargeIntents.merchantId, merchantId)];
  const q = parsed.data.q?.trim();
  if (q) conds.push(or(like(rechargeIntents.txHash, `%${q}%`), like(rechargeIntents.merchantOrderNo, `%${q}%`), like(rechargeIntents.id, `%${q}%`), like(rechargeIntents.address, `%${q}%`)));
  ...
  const totalRow = await db.select({ c: sql<number>`count(*)` }).from(rechargeIntents).where(where as any);
