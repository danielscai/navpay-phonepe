...
import { callbackTasks, payoutOrders } from "@/db/schema";
...
export async function POST(req: NextRequest, ctx: { params: Promise<{ orderId: string }> }) {
  ...
  if (!["SUCCESS", "FAILED", "REJECTED", "EXPIRED"].includes(String(o.status))) {
  ...
  await db.update(payoutOrders).set({ notifyStatus: "PENDING", lastNotifiedAtMs: null, updatedAtMs: Date.now() } as any).where(eq(payoutOrders.id, o.id));
