...
import { paymentPersonReportLogs } from "@/db/schema";
...
export async function GET(req: NextRequest, ctx: { params: Promise<{ personId: string }> }) {
  ...
  const totalRow = await db
    ...
    .from(paymentPersonReportLogs)
    .where(eq(paymentPersonReportLogs.personId, personId));
  ...
  const rows = await db
    ...
    .from(paymentPersonReportLogs)
    .where(eq(paymentPersonReportLogs.personId, personId))
    .orderBy(desc(paymentPersonReportLogs.createdAtMs))
