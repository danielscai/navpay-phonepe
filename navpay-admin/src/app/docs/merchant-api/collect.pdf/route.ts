...
export async function GET(req: NextRequest) {
  ...
  const endpoints = merchantApiV1Endpoints.filter((e) => e.id === "collect_create");
  const html = renderMerchantApiHtml({ title: "代收下单 API 文档 (V1)", endpoints, baseUrlHint });
  ...
  try {
    ...
    return new NextResponse(new Uint8Array(pdfBuf), {
      headers: {
        ...
        "content-disposition": "attachment; filename=\"navpay-merchant-api-collect-v1.pdf\"",
      },
    });
  } catch (e: any) {
